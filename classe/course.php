...
class course
{
    ...
    public static function SelectCourseAcheter($numMembre)
    {
        ...
        $requete = $bdd->prepare("SELECT `NumMembre`, `DateAchat`, `Course`.* FROM `achat` INNER JOIN `course` ON (achat.NumCourse = course.NumCourse) WHERE NumMembre = :membre AND panier = 0");
        $requete->bindValue(":membre", $numMembre);
        $requete->execute();
    ...
    public static function AjouterCoursePanier($numMembre, $numCourse)
    {
        ...
        $requete = $bdd->prepare("INSERT INTO `achat` (`NumMembre`, `NumCourse`, `Panier`, `DateAchat`) value(:membre, :course, :panier, :dateAchat)");
    ...
    public static function acheterCourse($numMembre, $numCourse)
    {
        ...
        $dateAchat = date("Y-d-m");

        $requete = $bdd->prepare("UPDATE `achat` SET `Panier`= 0, `DateAchat`=:dateAchat WHERE `NumMembre`= :membre AND `NumCourse`=:course");
